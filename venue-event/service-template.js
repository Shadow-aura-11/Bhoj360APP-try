...
app.get('/ai/event-recommendations', (req, res) => {
  const events = db.prepare('SELECT * FROM events').all();
  const recommendations = [
    {
      title: "Interactive Workshop Series",
      reason: "Based on high engagement in previous 'Tech Summit' breakout sessions.",
      potentialRevenue: "₹2.5L",
      suggestedFormat: "Hybrid"
    },
    {
      title: "Executive Networking Dinner",
      reason: "Significant overlap in high-tier ticket buyers for your last three conferences.",
      potentialRevenue: "₹4.8L",
      suggestedFormat: "In-person"
    }
  ];
  res.json(recommendations);
});

app.get('/ai/vendor-recommendations', (req, res) => {
  const categories = ['Catering', 'AV', 'Decor'];
  const recommendations = categories.map(cat => {
    const topVendor = db.prepare('SELECT * FROM vendors WHERE category = ? ORDER BY rating DESC LIMIT 1').get(cat);
    return {
      category: cat,
      recommendedVendor: topVendor ? topVendor.name : "Market Leader AI Suggestion",
      matchScore: "94%",
      reason: `Consistently high ratings and reliability for ${cat} in similar scale events.`
    };
  });
  res.json(recommendations);
});
