const RESTAURANT_ID = 'REST-NTCSBY';
const PORT = 3100;
/**
...
app.post('/travel-requests', authMiddleware('employee'), (req, res) => {
  ...
  // AI Feature: Basic Policy Compliance Check (Simulation)
  const policy = db.prepare('SELECT * FROM policies WHERE id = ?').get(req.employee.policy_id);
  let isCompliant = 1;
  let violations = [];
  ...
  if (policy) {
    ...
    if (days > 14) {
      isCompliant = 0;
      violations.push('Trip duration exceeds 14 days');
    }
  }

  db.prepare('INSERT INTO policy_compliance (request_id, policy_id, is_compliant, violations) VALUES (?, ?, ?, ?)')
    .run(request.id, req.employee.policy_id, isCompliant, JSON.stringify(violations));
...
app.post('/expenses', authMiddleware('employee'), (req, res) => {
  ...
  // AI Feature: Fraud Detection (Simulation)
  let fraudScore = Math.random() * 0.2; // Usually low
  let fraudNotes = 'Normal transaction pattern';

  if (amount > 5000) {
    fraudScore = 0.8 + Math.random() * 0.2;
    fraudNotes = 'Unusually high amount for category';
  }
  ...
  ).run(request_id, req.employee.id, category, amount, currency || 'USD', expense_date, receipt_url, fraudScore, fraudNotes);
...
app.get('/travel-requests/:id/ai-advice', authMiddleware(), (req, res) => {
  ...
  res.json({
    costOptimization: {
      recommendation: "Booking the flight 14 days in advance could save $240.",
      alternativeRoutes: ["LHR -> JFK via Dublin (Save $150)"]
    },
    riskAnalysis: {
      level: "Low",
